#!/usr/bin/env python3
"""
Phoneme-to-Grapheme (P2G) Training Script - FIXED
Based on: "LLM-based phoneme-to-grapheme for phoneme-based speech recognition"
Ma et al., Interspeech 2025

FIXES:
- Proper label handling (avoid -100 for all tokens)
- Disabled fp16 initially to debug
...
print("="*70)
print(" "*15 + "Phoneme-to-Grapheme (P2G) Training (FIXED)")
...
OUTPUT_DIR = "./p2g_model_polish_v2"
...
# STEP 1: LOAD DATA
# ============================================================================
print(f"\n[1/5] Loading phonemized TSV files...")

if USE_20H_SUBSET:
    train_tsv = os.path.join(TSV_DIR, "common_voice_pl_train_20h_phoneme_cleaned.tsv")
    print(f"   Using 20-hour subset")
else:
    train_tsv = os.path.join(TSV_DIR, "common_voice_pl_train_phoneme_cleaned.tsv")
    print(f"   Using full training data")

valid_tsv = os.path.join(TSV_DIR, "common_voice_pl_validation_phoneme_cleaned.tsv")
...
def preprocess_function(examples):
    ...
    # Tokenize targets (text) - CRITICAL: Don't use text_target for labels
...
# Data collator - this handles padding and label masking
data_collator = DataCollatorForSeq2Seq(
    tokenizer=tokenizer,
    model=model,
    padding=True,
    label_pad_token_id=-100  # Padding tokens in labels
)
...
training_args = Seq2SeqTrainingArguments(
    ...
    fp16=False,  # Disabled initially for debugging
    dataloader_num_workers=0,  # Disable to avoid fork warning
